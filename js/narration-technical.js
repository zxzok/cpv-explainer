// Generated by tools/build_narration.py from narration/technical.json -- do not edit.
window.CPV_NARRATION = {
 ...
 "chapters": [
  ...
  },
  {
   ...
   "read": {
    "en": "<p>The target is a <em>temporal aggregate</em> of a latent trajectory, <span class='f'>Θ = \\int ω(t)\\,g(Z(t))\\,\\text{d}t</span>: with <span class='f'>g(z)=z</span> an average level, with <span class='f'>g(z) = \\mathbf{1}\\{z &gt; c\\}</span> an occupation time such as REM proportion or AF burden. A protocol is a finite set of noisy linear acquisition actions under a cost budget. Weak benchmark performance may reflect the learner, the sample size, or target-relevant information the protocol never admitted. Only the third is addressed by changing what is observed — and whether the benchmark can even evaluate that change is the identification question of Section 3.</p>",
    "zh": "<p>目标是潜在轨迹的<em>时间聚合量</em>：<span class='f'>Θ = \\int ω(t)\\,g(Z(t))\\,\\text{d}t</span>。取 <span class='f'>g(z)=z</span> 是平均水平；取 <span class='f'>g(z) = \\mathbf{1}\\{z &gt; c\\}</span> 是占用时间，例如 REM 比例或房颤负担。协议是预算约束下的一组带噪声线性测量动作。基准上表现差，可能源于模型、样本量，或协议根本没有采到的目标相关信息。只有第三种需要改变观测本身——而基准数据能否评估这种改变，正是第 3 节的识别问题。</p>"
   }
  },
  {
   "id": "ch2",
   ...
   "beats": [
    ...
    },
    {
     ...
     "zh_tts": "在高斯模型里，这件事变得很具体。潜在轨迹在一组时间格点上有一个相关矩阵 K。目标是对状态做某种变换之后的加权平均，权重是 欧米伽。而一个协议，只观测少数几个带噪声的线性测量，由它的测量矩阵 A 和噪声 R 来描述。",
     "dur": {
      "en": 23.4,
      "zh": 20.27
     }
   ...
   "read": {
    "en": "<p><strong>Definition 1.</strong> The Bayes value of protocol <span class='f'>S</span> is <span class='f'>I(S) = \\frac{Var(E[Θ \\mid Y_S])}{Var(Θ)}</span>, the population R² of the optimal predictor. Under the discrete Gaussian model <span class='f'>Z \\sim N(0, K)</span>, <span class='f'>diag(K)=1</span>, with measurements <span class='f'>Y_S = A_SZ + ε</span>, <strong>Proposition 2</strong> gives the closed form <span class='f'>I_g(S;K) = \\frac{F_g(S;K)}{V_g(K)}</span>, where <span class='f'>Q_S(K) = K A_S^\\top (A_S K A_S^\\top + R_S)^{-1} A_S K</span> is the covariance of the recovered trajectory and <span class='f'>C_g(r) = Cov\\{g(U), g(V_r)\\}</span> is the Gaussian covariance transform of the target function (for the mean target <span class='f'>C_g(r)=r</span>; for occupation above 0, <span class='f'>\\frac{arcsin(r)}{2π}</span>). Every nonlinearity of the target enters through <span class='f'>C_g</span> alone.</p>",
    "zh": "<p><strong>定义 1.</strong> 协议 <span class='f'>S</span> 的贝叶斯价值是 <span class='f'>I(S) = \\frac{Var(E[Θ \\mid Y_S])}{Var(Θ)}</span>，即最优预测器的总体 R²。在离散高斯模型 <span class='f'>Z \\sim N(0, K)</span>、<span class='f'>diag(K)=1</span>、测量 <span class='f'>Y_S = A_SZ + ε</span> 下，<strong>命题 2</strong> 给出闭式 <span class='f'>I_g(S;K) = \\frac{F_g(S;K)}{V_g(K)}</span>：其中 <span class='f'>Q_S(K) = K A_S^\\top (A_S K A_S^\\top + R_S)^{-1} A_S K</span> 是从测量恢复出的那部分轨迹的协方差，<span class='f'>C_g(r) = Cov\\{g(U), g(V_r)\\}</span> 是目标函数的高斯协方差变换（均值目标 <span class='f'>C_g(r)=r</span>；零阈值占用时间为 <span class='f'>\\frac{arcsin(r)}{2π}</span>）。目标的全部非线性只通过 <span class='f'>C_g</span> 进入。</p>"
   }
  },
  {
   "id": "ch3",
   "title": {
    "en": "Two worlds, one benchmark",
    "zh": "两个世界，同一基准"
   },
   "kicker": {
    "en": "Section 3 · Theorem 3",
    "zh": "第 3 节 · 定理 3"
   },
   "beats": [
    {
    ...
    },
    {
     ...
     "zh_tts": "我们从相关函数 e 的负滞后次方出发，沿着零、一、负二、一这个方向，一边加上 艾普西隆，一边减去 艾普西隆。这样就得到两个世界：K 加和 K 减。你可以看到，它们的时间依赖结构明显不一样。",
     "dur": {
      "en": 20.92,
      "zh": 17.0
     }
    ...
    },
    {
     ...
     "zh_tts": "你可以自己拖一下 艾普西隆。三个可观测的数纹丝不动，而协议 B 的价值在滑动。这就是定理三。而且四个点是一个精确的门槛：只有三个点或者更少的时候，同样的基准数据就足以确定整个平稳相关函数。",
     "dur": {
      "en": 16.52,
      "zh": 19.31
     }
   ...
   "read": {
    "en": "<p><strong>Theorem 3 (sharp minimal stationary counterexample).</strong> For a stationary standardised process on <span class='f'>p</span> points, a uniform mean target and a one-point benchmark, the stationary invisible space has dimension <span class='f'>p − 3</span>: identification holds for <span class='f'>p ≤ 3</span> and fails from <span class='f'>p = 4</span>. At <span class='f'>p = 4</span> the kernel is spanned by the lag perturbation <span class='f'>δ = (0, 1, −2, 1)</span>, and for <span class='f'>B = \\{Z_1, Z_2\\}</span> the two values are <span class='f'>I(B;K_±) = \\frac{2b^2}{(1 + ν_B^2 + ρ(1) ± ε)\\,Var(Θ)}</span>, <span class='f'>b = \\frac{1 + 2ρ(1) + ρ(2)}{4}</span>. Figure 1 of the paper uses <span class='f'>ρ_0(u) = e^{−u}</span> and <span class='f'>ε = 0.1321</span>: both worlds give <span class='f'>Var(Y_A) = 1</span>, <span class='f'>Cov(Y_A, Θ) = 0.388250</span>, <span class='f'>Var(Θ) = 0.428012</span> to <span class='f'>10^{-16}</span>, yet <span class='f'>I(B;K_{+}) = 0.6817</span> and <span class='f'>I(B;K_{−}) = 0.8274</span>. Along <span class='f'>δ</span> the target's covariance with <span class='f'>Z_1</span> and <span class='f'>Z_2</span> is fixed; only <span class='f'>Cov(Z_1, Z_2)</span> moves — the redundancy between B's two measurements.</p>",
    "zh": "<p><strong>定理 3（精确的最小平稳反例）.</strong> 对 <span class='f'>p</span> 个格点上的平稳标准化过程、均匀均值目标和单点基准，平稳不可见空间的维数是 <span class='f'>p − 3</span>：<span class='f'>p ≤ 3</span> 时可识别，<span class='f'>p = 4</span> 起失效。<span class='f'>p = 4</span> 时核由滞后扰动 <span class='f'>δ = (0, 1, −2, 1)</span> 张成；对 <span class='f'>B = \\{Z_1, Z_2\\}</span>，两个价值为 <span class='f'>I(B;K_±) = \\frac{2b^2}{(1 + ν_B^2 + ρ(1) ± ε)\\,Var(Θ)}</span>，<span class='f'>b = \\frac{1 + 2ρ(1) + ρ(2)}{4}</span>。论文图 1 取 <span class='f'>ρ_0(u) = e^{−u}</span>、<span class='f'>ε = 0.1321</span>：两个世界的 <span class='f'>Var(Y_A) = 1</span>、<span class='f'>Cov(Y_A, Θ) = 0.388250</span>、<span class='f'>Var(Θ) = 0.428012</span> 一致到 <span class='f'>10^{-16}</span>，而 <span class='f'>I(B;K_{+}) = 0.6817</span>、<span class='f'>I(B;K_{−}) = 0.8274</span>。沿 <span class='f'>δ</span> 移动时，目标与 <span class='f'>Z_1</span>、<span class='f'>Z_2</span> 的协方差不变，只有 <span class='f'>Cov(Z_1, Z_2)</span>——B 两个测量之间的冗余——在变。</p>"
   }
  },
  {
   "id": "ch4",
   "title": {
    "en": "Invisible directions",
    "zh": "不可见方向"
   },
   "kicker": {
    "en": "Section 3 · Theorem 5, Proposition 6",
    "zh": "第 3 节 · 定理 5、命题 6"
   },
   "beats": [
    {
    ...
    },
    {
     ...
     "zh_tts": "定理五把这件事变成一个真正可以执行的检验。如果协议 B 的价值沿着某个不可见方向的导数不为零，那么这个价值就不可识别：K 零加减 艾普西隆 德尔塔 这两个世界，产生同样的基准分布，却对协议 B 给出不同的答案。",
     "dur": {
      "en": 20.43,
      "zh": 19.92
     }
   ...
   "read": {
    "en": "<p><strong>Definition 4 / Theorem 5.</strong> A symmetric zero-diagonal <span class='f'>Δ</span> is <em>invisible</em> to <span class='f'>(A, h)</span> if <span class='f'>AΔA^\\top = 0</span>, <span class='f'>AΔh = 0</span>, <span class='f'>h^\\topΔh = 0</span>. For every small <span class='f'>ε</span>, <span class='f'>K_0 ± εΔ</span> are correlation matrices with the same benchmark law. If the directional derivative <span class='f d'>D_B(Δ;K_0) = \\frac{2h^\\top Δ B^\\top W B K_0 h − h^\\top K_0 B^\\top W (B Δ B^\\top) W B K_0 h}{h^\\top K_0 h}</span> (<span class='f'>W = (B K_0 B^\\top + R_B)^{-1}</span>) is non-zero, the value of <span class='f'>B</span> is not locally identified. Rank–nullity gives <span class='f'>dim\\, D(A, h) ≥ p(p − 1)/2 − d(d + 1)/2 − d − 1</span>: <span class='f'>O(d^2)</span> constraints against <span class='f'>O(p^2)</span> parameters. <strong>Proposition 6 / Example 7.</strong> For arbitrary <span class='f'>g ∈ L^2(φ)</span>, a permutation <span class='f'>Ξ</span> with <span class='f'>AΞ = A</span> and <span class='f'>Ξ^\\topω = ω</span> makes the laws of <span class='f'>(Y_A, Θ_g)</span> under <span class='f'>K</span> and <span class='f'>ΞKΞ^\\top</span> coincide exactly while <span class='f'>I_g(B; ΞKΞ^\\top) = I_g(BΞ; K)</span>; three points suffice under uniform aggregation.</p>",
    "zh": "<p><strong>定义 4 / 定理 5.</strong> 对称、零对角的 <span class='f'>Δ</span> 对 <span class='f'>(A, h)</span> <em>不可见</em>，若 <span class='f'>AΔA^\\top = 0</span>、<span class='f'>AΔh = 0</span>、<span class='f'>h^\\topΔh = 0</span>。对足够小的 <span class='f'>ε</span>，<span class='f'>K_0 ± εΔ</span> 都是相关矩阵且基准分布相同。若方向导数 <span class='f d'>D_B(Δ;K_0) = \\frac{2h^\\top Δ B^\\top W B K_0 h − h^\\top K_0 B^\\top W (B Δ B^\\top) W B K_0 h}{h^\\top K_0 h}</span>（<span class='f'>W = (B K_0 B^\\top + R_B)^{-1}</span>）非零，则 <span class='f'>B</span> 的价值局部不可识别。秩–零化度给出 <span class='f'>dim\\, D(A, h) ≥ p(p − 1)/2 − d(d + 1)/2 − d − 1</span>：<span class='f'>O(d^2)</span> 个约束对 <span class='f'>O(p^2)</span> 个参数。<strong>命题 6 / 例 7.</strong> 对任意 <span class='f'>g ∈ L^2(φ)</span>，满足 <span class='f'>AΞ = A</span>、<span class='f'>Ξ^\\topω = ω</span> 的置换 <span class='f'>Ξ</span> 使 <span class='f'>K</span> 与 <span class='f'>ΞKΞ^\\top</span> 下 <span class='f'>(Y_A, Θ_g)</span> 的分布完全相同，而 <span class='f'>I_g(B; ΞKΞ^\\top) = I_g(BΞ; K)</span>；均匀聚合下三个格点即可。</p>"
   }
  },
  {
   "id": "ch5",
   "title": {
    "en": "Restoring identification",
    "zh": "恢复可识别性"
   },
   "kicker": {
    "en": "Sections 3.5–4 · Proposition 8, Theorems 10–11",
    "zh": "第 3.5–4 节 · 命题 8、定理 10–11"
   },
   "beats": [
    {
     "id": "b0",
   ...
   "read": {
    "en": "<p><strong>Proposition 8 (augmentation).</strong> If the row space of <span class='f'>B</span> lies in the row space of the augmented benchmark together with <span class='f'>h^\\top</span>, the value of <span class='f'>B</span> is identified exactly; a non-zero derivative along an augmented-invisible direction certifies failure. <strong>Calibration (Section 4).</strong> With <span class='f'>m</span> densely observed units <span class='f'>W = Z + η</span>, the estimator is <span class='f'>\\hat K = R_τ(\\hat Σ − \\hat R_0)</span> — deconvolution, eigenvalue floor, standardisation — and <span class='f'>\\hat I_g(S) = F_g(S;\\hat K)/V_g(\\hat K)</span>. <strong>Theorem 10.</strong> Uniformly over a conditioned family, <span class='f'>\\sup_S |\\hat I_g(S) − I_g(S)| ≤ C\\,\\norm{\\hat K − K}^{β}</span>, with <span class='f'>β = 1</span> for <span class='f'>g ∈ W^{1,2}(φ)</span>, <span class='f'>β = 1/2</span> for threshold targets globally (sharp, Proposition 9), and <span class='f'>β = 1</span> at any fixed interior model. <strong>Theorem 11.</strong> Hence the plug-in values converge at the root-<span class='f'>m</span> rate at a fixed model. Figure 2(a): uniform error over 495 four-action protocols, slopes −0.413 (mean) and −0.414 (occupation at 0); over the three largest <span class='f'>m</span>, −0.462 and −0.464.</p>",
    "zh": "<p><strong>命题 8（补测）.</strong> 若 <span class='f'>B</span> 的行空间落在补测后的基准与 <span class='f'>h^\\top</span> 所张成的行空间内，则 <span class='f'>B</span> 的价值被精确识别；沿补测后仍不可见的方向导数非零，则证明失败。<strong>校准（第 4 节）.</strong> 用 <span class='f'>m</span> 个密集观测对象 <span class='f'>W = Z + η</span>，估计量为 <span class='f'>\\hat K = R_τ(\\hat Σ − \\hat R_0)</span>——去噪、特征值下限、标准化——再取 <span class='f'>\\hat I_g(S) = F_g(S;\\hat K)/V_g(\\hat K)</span>。<strong>定理 10.</strong> 在条件数受控的候选族上一致地有 <span class='f'>\\sup_S |\\hat I_g(S) − I_g(S)| ≤ C\\,\\norm{\\hat K − K}^{β}</span>：<span class='f'>g ∈ W^{1,2}(φ)</span> 时 <span class='f'>β = 1</span>；阈值目标全局 <span class='f'>β = 1/2</span>（命题 9 证明其不可改进）；在任何固定内部模型上 <span class='f'>β = 1</span>。<strong>定理 11.</strong> 因而固定模型下的插入估计以根号 <span class='f'>m</span> 速率收敛。图 2(a)：495 个四动作协议上的一致误差，斜率 −0.413（均值）与 −0.414（零阈值占用时间）；最大的三个 <span class='f'>m</span> 上为 −0.462 与 −0.464。</p>"
   }
  },
  {
   "id": "ch6",
   ...
   "beats": [
    {
     ...
     "zh_tts": "接下来，一致误差就变成了一个决策保证，而且只需要三行。真正最优的协议可能被低估，但最多低估 艾普西隆；我们选中的那个可能被高估，也最多高估 艾普西隆。所以按估计值来选，损失最多是二倍 艾普西隆。比这个尺度更小的价值差距，根本分辨不出来。",
     "dur": {
      "en": 22.76,
      "zh": 23.53
     }
    },
    {
     ...
     "zh_tts": "候选类可以很粗，比如只分“连续还是分散”；也可以很细，细到精确的锚点集合。粗的类可能根本不包含最优解；细的类包含它，却更难分辨。总的损失，就是类别限制的损失，再加上二倍 艾普西隆。",
     "dur": {
      "en": 17.51,
      "zh": 17.95
     }
   ...
   "read": {
    "en": "<p><strong>Corollary 12.</strong> With <span class='f'>ε_m = \\sup_S |\\hat I_g(S) − I_g(S)|</span>, an empirical maximiser <span class='f'>\\tilde S</span> with optimisation gap <span class='f'>η_m</span> satisfies <span class='f'>I_g(S^*) − I_g(\\tilde S) ≤ 2ε_m + η_m</span>. For nested classes <span class='f'>Π^{(1)} ⊆ … ⊆ Π^{(L)}</span>, the regret of the best estimated protocol in class <span class='f'>ℓ</span> is at most the <em>class restriction</em> <span class='f'>I(S^*) − \\max_{Π^{(ℓ)}} I</span> plus <span class='f'>2ε_ℓ</span>. Figure 2(b–d): across 7200 checks the realised regret never exceeds <span class='f'>2ε_m</span> (median ratio 0.024, maximum 0.37); regret falls at slope −0.94 while the envelope falls at −0.49; in four nested classes of sizes 2, 5, 75 and 568, the finest class has regret 0.037 at <span class='f'>m = 25</span> against 0.051 for the coarsest, and at <span class='f'>m = 1000</span> the coarsest is still at 0.037 while the finest reaches 0.006.</p>",
    "zh": "<p><strong>推论 12.</strong> 记 <span class='f'>ε_m = \\sup_S |\\hat I_g(S) − I_g(S)|</span>，则优化间隙为 <span class='f'>η_m</span> 的经验极大化解 <span class='f'>\\tilde S</span> 满足 <span class='f'>I_g(S^*) − I_g(\\tilde S) ≤ 2ε_m + η_m</span>。对嵌套类 <span class='f'>Π^{(1)} ⊆ … ⊆ Π^{(L)}</span>，第 <span class='f'>ℓ</span> 类中最优估计协议的损失不超过<em>类别限制</em> <span class='f'>I(S^*) − \\max_{Π^{(ℓ)}} I</span> 加 <span class='f'>2ε_ℓ</span>。图 2(b–d)：7200 次检验中实际损失从未超过 <span class='f'>2ε_m</span>（比值中位数 0.024，最大 0.37）；损失以 −0.94 的斜率下降而包络以 −0.49 下降；在大小为 2、5、75、568 的四个嵌套类中，<span class='f'>m = 25</span> 时最细类的损失为 0.037，最粗类为 0.051；<span class='f'>m = 1000</span> 时最粗类仍停在 0.037，而最细类降到 0.006。</p>"
   }
  },
  {
   "id": "ch7",
   "title": {
    "en": "Target-aware design",
    "zh": "针对目标的观测设计"
   },
   "kicker": {
    "en": "Section 5 · Proposition 13, Lemma 14",
    "zh": "第 5 节 · 命题 13、引理 14"
   },
   "beats": [
    {
     "id": "b0",
   ...
   "read": {
    "en": "<p><strong>Proposition 13 (rank-one marginal gains).</strong> With residual covariance <span class='f'>P_S = K − Q_S = Cov(Z \\mid Y_S)</span>, adding action <span class='f'>a = (ℓ_a, r_a, c_a)</span> gives <span class='f'>v = P_Sℓ_a</span>, <span class='f'>s = ℓ_a^\\top P_S ℓ_a + r_a</span>, <span class='f'>Q_{S ∪ a} = Q_S + v v^\\top / s</span>, and the exact gain <span class='f d'>Δ_g(a \\mid S) = \\sum_{j,k} ω_j ω_k \\,[\\, C_g(Q_{jk} + v_j v_k / s) − C_g(Q_{jk}) \\,]</span>; for the mean target <span class='f'>Δ = \\frac{(ω^\\top P_S ℓ_a)^2}{ℓ_a^\\top P_S ℓ_a + r_a}</span>. <strong>Lemma 14.</strong> <span class='f'>F_g</span> is monotone under nested protocols, but not submodular (R² subset selection is not). Table 1: over 25 enumerated instances, relative efficiency against the exhaustive target-specific optimum has minimum 0.601 (latent mutual information), 0.802 (integrated posterior variance), 0.767 (linear target with actual noise), 0.231 (noiseless kernel quadrature), 0.912 (target-aware greedy) and 0.987 (greedy + one swap), the last while evaluating 9–23% of the enumerated sets.</p>",
    "zh": "<p><strong>命题 13（秩一边际收益）.</strong> 记残差协方差 <span class='f'>P_S = K − Q_S = Cov(Z \\mid Y_S)</span>，加入动作 <span class='f'>a = (ℓ_a, r_a, c_a)</span> 时 <span class='f'>v = P_Sℓ_a</span>、<span class='f'>s = ℓ_a^\\top P_S ℓ_a + r_a</span>、<span class='f'>Q_{S ∪ a} = Q_S + v v^\\top / s</span>，精确收益为 <span class='f d'>Δ_g(a \\mid S) = \\sum_{j,k} ω_j ω_k \\,[\\, C_g(Q_{jk} + v_j v_k / s) − C_g(Q_{jk}) \\,]</span>；均值目标下 <span class='f'>Δ = \\frac{(ω^\\top P_S ℓ_a)^2}{ℓ_a^\\top P_S ℓ_a + r_a}</span>。<strong>引理 14.</strong> <span class='f'>F_g</span> 对嵌套协议单调，但不是次模的（R² 子集选择本身就不是）。表 1：25 个穷举实例上，相对穷举最优的效率最小值依次为 0.601（潜在互信息）、0.802（积分后验方差）、0.767（带真实噪声的线性目标）、0.231（无噪声核求积）、0.912（针对目标的贪心）、0.987（贪心加一轮交换），最后一项只评估了穷举集合的 9–23%。</p>"
   }
  },
  {
   "id": "ch8",
   "title": {
    "en": "Sleep and atrial fibrillation",
    "zh": "睡眠与房颤"
   },
   "kicker": {
    "en": "Section 6.4 · Figures 3–4",
    "zh": "第 6.4 节 · 图 3–4"
   },
   "beats": [
    {
   ...
   "read": {
    "en": "<p>Both analyses use only expert annotation files — 197 Sleep-EDF hypnograms (100 subjects, 3818 valid hours, <span class='f'>p = 128</span> relative-time anchors) and 84 Long-Term AF records (1934 hours, <span class='f'>p = 96</span> bins) — with subject-disjoint five-fold cross-fitting and pooled held-out R². <strong>Sleep (REM):</strong> centred-contiguous 0.648 / 0.659 / 0.863 versus uniformly dispersed 0.682 / 0.738 / 0.881 at <span class='f'>N = 4, 16, 64</span>; 9 of 15 target–budget cells have a dispersed-minus-contiguous percentile range above zero, but SC and ST source studies disagree (10 above, 5 below, 15 spanning zero of 30 cells). <strong>AF burden:</strong> contiguous versus dispersed 0.696 vs 0.971 at one hour (<span class='f'>N = 4</span> windows) and 0.851 vs 0.998 at four hours; the paired range is positive at every multi-window budget. <strong>Learned supports:</strong> the target-aware REM support at <span class='f'>N = 16</span> scores 0.694 against 0.650 (learned kernel quadrature) and 0.738 (fixed dispersion); over 1000 study-stratified 80% subsamples the differences have medians −0.014 and −0.012 with ranges [−0.102, +0.078] and [−0.126, +0.097]. Coarse layout contrasts were clearer than exact learned anchors — the resolution pattern of Figure 2 seen in real data.</p>",
    "zh": "<p>两项分析只使用专家标注文件——197 段 Sleep-EDF 分期（100 名受试者、3818 有效小时、<span class='f'>p = 128</span> 个相对时间锚点）与 84 段长程房颤记录（1934 小时、<span class='f'>p = 96</span> 个分箱）——采用受试者不交叠的五折交叉拟合与汇总留出 R²。<strong>睡眠（REM）：</strong> <span class='f'>N = 4, 16, 64</span> 时居中连续块为 0.648 / 0.659 / 0.863，均匀分散为 0.682 / 0.738 / 0.881；15 个目标–预算格中有 9 个“分散减连续”的分位数区间高于零，但 SC 与 ST 两个来源研究并不一致（30 格中 10 个高于零、5 个低于零、15 个跨零）。<strong>房颤负担：</strong> 一小时（<span class='f'>N = 4</span> 个窗口）连续对分散为 0.696 对 0.971，四小时为 0.851 对 0.998；每个多窗口预算上配对区间都为正。<strong>学得的位置：</strong> <span class='f'>N = 16</span> 时针对目标的 REM 位置得分 0.694，对比学得核求积 0.650 与固定分散 0.738；在 1000 次按研究分层的 80% 重抽样中，差值中位数为 −0.014 与 −0.012，区间为 [−0.102, +0.078] 与 [−0.126, +0.097]。粗布局的对比比精确学得的锚点更清楚——这正是图 2 的分辨率规律在真实数据上的体现。</p>"
   }
  },
  {
   "id": "ch9",
   "title": {
    "en": "Identification before optimisation",
    "zh": "先识别，再优化"
   },
   "kicker": {
    "en": "Section 8",
